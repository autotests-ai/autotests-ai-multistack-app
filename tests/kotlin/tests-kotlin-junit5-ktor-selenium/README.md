# tests-kotlin-junit5-ktor-selenium

Gradle · JUnit 5 · Allure 3 · **Selenium 4** · in-cell **Ktor** (`api` + `ui` + `e2e`).

Kotlin UI school (raw WebDriver, not Selenide). HTTP in this folder is Ktor (same client as `tests-kotlin-junit5-ktor`), not a second clone folder. Default CI cell stays [`tests-java-junit5-rest_assured-selenide`](../../java/tests-java-junit5-rest_assured-selenide/). Java Selenium canon stays [`tests-java-junit5-rest_assured-selenium`](../../java/tests-java-junit5-rest_assured-selenium/). HTTP-only Ktor school stays in the sibling folder. Not a Playwright rewrite.
...
CI `sonar-tests` uses the Gradle Sonar plugin (`projectKey` `autotests-ai-multistack-app-tests-kotlin-junit5-ktor-selenium`, gate `qa-guru-canon`). Do not flip clone `TESTS_LANG` to kotlin.
