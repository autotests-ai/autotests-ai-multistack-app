# tests-kotlin-junit5-api_request-playwright

Gradle · JUnit 5 · Allure 3 · **Playwright for Java** UI · in-cell **APIRequest** (`api` + `ui` + `e2e`).

Kotlin Playwright school. Page objects use `getByTestId` / `Locator` behind `pages/App` (`app.login` / `app.header` / `app.home`). HTTP in this folder is Playwright `APIRequest`, not Ktor. Default CI cell stays [`tests-java-junit5-rest_assured-selenide`](../../java/tests-java-junit5-rest_assured-selenide/). Java Playwright canon stays [`tests-java-junit5-api_request-playwright`](../../java/tests-java-junit5-api_request-playwright/). HTTP-only Ktor school stays in the sibling folder. Not a Selenide/Selenium rewrite.
...
```bash
cd tests/kotlin/tests-kotlin-junit5-api_request-playwright
...
| api | `./gradlew test -Denv=ci -DincludeTags=api` | in-cell APIRequest · same `/api` catalog as the HTTP-only Ktor cell |
...
CI `sonar-tests` uses the Gradle Sonar plugin (`projectKey` `autotests-ai-multistack-app-tests-kotlin-junit5-api_request-playwright`, gate `qa-guru-canon`). Do not flip clone `TESTS_LANG` to kotlin.
