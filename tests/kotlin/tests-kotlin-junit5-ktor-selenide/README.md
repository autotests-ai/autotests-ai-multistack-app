# tests-kotlin-junit5-ktor-selenide

Gradle · JUnit 5 · Allure 3 · **Selenide** · in-cell **Ktor** (`api` + `ui` + `e2e`).

Kotlin UI school. HTTP in this folder is Ktor (same client as `tests-kotlin-junit5-ktor`), not a second clone folder. Default CI cell stays [`tests-java-junit5-rest_assured-selenide`](../../java/tests-java-junit5-rest_assured-selenide/). HTTP-only Ktor school stays in the sibling folder.
...
CI `sonar-tests` uses the Gradle Sonar plugin (`projectKey` `autotests-ai-multistack-app-tests-kotlin-junit5-ktor-selenide`, gate `qa-guru-canon`). Do not flip clone `TESTS_LANG` to kotlin.
