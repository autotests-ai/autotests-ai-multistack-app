...
Report: `build/reports/gatling/`. JMeter JMX is the **etalon living** cell: [`tests-java-jmeter`](../../java/tests-java-jmeter/). Java DSL sibling: [`tests-java-gatling`](../../java/tests-java-gatling/). Scala DSL sibling: [`tests-scala-gatling`](../../scala/tests-scala-gatling/). JS / TS SDKs stay slots. Teaching job is `load-tests` (`LOAD_LANG: kotlin`, `LOAD_TOOL: gatling`) — not a `gatling-*` box. Student emit: `kotlin-gatling` (templates still planned — do not copy this folder).
