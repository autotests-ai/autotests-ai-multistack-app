...
Report: `build/reports/gatling/`. JMeter JMX is the **etalon living** cell: [`tests-java-jmeter`](../../java/tests-java-jmeter/). Java DSL sibling: [`tests-java-gatling`](../../java/tests-java-gatling/). Scala / JS / TS SDKs stay slots. Teaching `ci.yml` does not name Gatling jobs — run this folder locally (or against the isolated load host). Student emit: `kotlin-gatling` (templates still planned — do not copy this folder).
