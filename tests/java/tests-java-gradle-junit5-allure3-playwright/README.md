...
Same `data-testid` stems as the TypeScript Playwright living cell. Page objects use `getByTestId` / `Locator` behind `pages/App` (`app.login` / `app.header` / `app.home`). HTTP is Rest Assured. The HTTP-only Rest Assured school stays in the sibling folder.
