#!/usr/bin/env node
/**
 * Second Allure 3 generate pass — pipeline:
 *   generate (dashboardTheme) → notifications (collage/Telegram).
 * Pass 1: gradlew allureReport (summary.json on disk).
 * Theme is intermediate (colors-only via @pyramid); not dashboard-overrides.js.
 */
