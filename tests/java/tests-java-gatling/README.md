...
Report: `build/reports/gatling/`. Sibling JMeter JMX is the **etalon living** cell: [`tests-java-jmeter`](../tests-java-jmeter/). Kotlin DSL sibling: [`tests-kotlin-gatling`](../../kotlin/tests-kotlin-gatling/). CI: named jobs `gatling-java-cell` / `gatling-kotlin-cell` (`gatling-load-cell`, not a matrix) against `load.autotests.ai` java/kotlin Spring mounts. Student emit: `java-gatling` (templates still planned — do not copy this folder).
