...
Report: `build/reports/gatling/`. Sibling JMeter JMX is the **etalon living** cell: [`tests-java-jmeter`](../tests-java-jmeter/). Kotlin DSL sibling: [`tests-kotlin-gatling`](../../kotlin/tests-kotlin-gatling/). Scala DSL sibling: [`tests-scala-gatling`](../../scala/tests-scala-gatling/). Teaching job is `load-tests` (`LOAD_TOOL: gatling`) — not a `gatling-*` box; HTML goes to [load.autotests.ai/runs/](https://load.autotests.ai/) like JMeter. Student emit: `java-gatling` (templates still planned — do not copy this folder).
