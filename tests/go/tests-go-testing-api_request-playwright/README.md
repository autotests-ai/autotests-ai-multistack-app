# tests-go-testing-api_request-playwright

`go test` · **playwright-go** · in-cell **APIRequest** (`api` + `ui` + `e2e`) · [testify](https://github.com/stretchr/testify) · [official Allure Go](https://github.com/allure-framework/allure-go).

Go UI school (`GetByTestId`, same `data-testid` as Java/TS Playwright). HTTP in this folder is Playwright `APIRequest`, not net/http. Default CI cell stays [`tests-java-junit5-rest_assured-selenide`](../../java/tests-java-junit5-rest_assured-selenide/). Mill stays [`tests-go-cdp`](../tests-go-cdp/). Not a generate sibling.

```bash
cd tests/go/tests-go-testing-api_request-playwright
...
(`projectKey` `autotests-ai-multistack-app-tests-go-testing-api_request-playwright`, gate `qa-guru-canon`).
