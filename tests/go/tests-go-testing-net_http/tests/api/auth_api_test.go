...
func TestRegisterAcceptsMinimumLengthCredentials(t *testing.T) {
	wrapAuth(t, "POST /api/auth/register accepts a 3-character username and 6-character password", func(a *allure.Context) {
		name := tests.UsernameAtMinLength()
		res := tests.Request(t, a, http.MethodPost, "/api/auth/register", tests.RequestOpt{
			JSON: map[string]string{"username": name, "password": tests.PasswordAtMinLength()},
		})
		require.Equal(t, http.StatusCreated, res.Status)
		tests.AssertSchema(t, res.Raw, "auth-response.json")
		body := res.Map(t)
...
func TestLoginMinLengthUnknownUserIsUnauthorized(t *testing.T) {
	wrapAuth(t, "POST /api/auth/login with min-length unknown user is 401, not 400", func(a *allure.Context) {
		res := tests.Request(t, a, http.MethodPost, "/api/auth/login", tests.RequestOpt{
			JSON: map[string]string{
				"username": tests.UsernameAtMinLength(),
				"password": tests.PasswordAtMinLength(),
			},
		})
		require.Equal(t, http.StatusUnauthorized, res.Status)
		tests.AssertSchema(t, res.Raw, "error.json")
