#!/usr/bin/env bash
# JaCoCo analog: 100% on ConfigReader helpers + LayoutCss + TokensCss (not UI driver).
set -euo pipefail
cd "$(dirname "$0")"
...
cargo llvm-cov --test infra --json --output-path coverage.json \
  --ignore-filename-regex '(client|schema|meta|lib|ui|pages|webdriver|viewport|mock_scenarios|user|screenshot|local_chrome_pin)\.rs$'
...
if fail:
    raise SystemExit(1)
print("ConfigReader/LayoutCss/TokensCss analog: " + " ".join(need) + " at 100.0%")
PY
