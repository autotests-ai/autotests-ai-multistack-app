# tests-csharp-xunit-allure3-playwright

xUnit.net · **Playwright** · [Allure.Xunit](https://www.nuget.org/packages/Allure.Xunit) · in-cell **RestSharp** (`api` + `ui` + `e2e`).

C# UI school (Playwright `GetByTestId`, same `data-testid` as Java/TS Playwright). HTTP in this folder is RestSharp (same client as `tests-csharp-nunit-allure3-restsharp`), not a second clone folder. Default CI cell stays [`tests-java-gradle-junit5-allure3-selenide`](../../java/tests-java-gradle-junit5-allure3-selenide/). Java/Kotlin/Python Playwright canons stay in their folders. HTTP-only RestSharp school stays in the sibling folder. Not an NUnit/Selenium rewrite.
...
(`projectKey` `autotests-ai-multistack-app-tests-csharp-xunit-allure3-playwright`, gate `qa-guru-canon`).
