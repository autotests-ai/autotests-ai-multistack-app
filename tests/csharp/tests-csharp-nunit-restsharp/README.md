# tests-csharp-nunit-restsharp

NUnit · **RestSharp** · [Allure.NUnit](https://www.nuget.org/packages/Allure.NUnit). HTTP-only school — same `/api` catalog as Java Rest Assured (31 api + 9 ConfigReader). No browser, no manual layer.

Sibling UI school: [`tests-csharp-nunit-restsharp-selenium`](../tests-csharp-nunit-restsharp-selenium/) (Selenium + **in-cell** RestSharp). xUnit Playwright stays a slot. Combo with that UI school = generate, not a third folder.

```bash
cd tests/csharp/tests-csharp-nunit-restsharp
