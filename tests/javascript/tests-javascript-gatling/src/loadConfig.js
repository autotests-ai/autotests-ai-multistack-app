/**
 * Stand + injection knobs for the Gatling JS SDK (Graal, not Node APIs).
...
export function profile() {
  return firstNonBlank(
    ...
    getEnvironmentVariable("GATLING_PROFILE"),
    "smoke",
  ).toLowerCase();
}
...
export function users() {
  return Math.max(
    1,
    parseIntOr(firstNonBlank(getParameter("users"), getEnvironmentVariable("GATLING_USERS"), "1"), 1),
  );
}
...
export function duringSeconds() {
  return Math.max(
    1,
    parseIntOr(
      firstNonBlank(getParameter("duringSeconds"), getEnvironmentVariable("GATLING_DURING_SECONDS"), "30"),
      30,
    ),
  );
}
...
export function p95Ms() {
  return Math.max(
    1,
    parseIntOr(firstNonBlank(getParameter("p95Ms"), getEnvironmentVariable("GATLING_P95_MS"), "2000"), 2000),
  );
}
...
export function allowPublic() {
  return (
    firstNonBlank(
      getParameter("allowPublic"),
      getEnvironmentVariable("GATLING_ALLOW_PUBLIC"),
      "false",
    ).toLowerCase() === "true"
  );
}
