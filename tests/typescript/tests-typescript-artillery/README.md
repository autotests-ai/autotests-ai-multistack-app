...
Smoke is **1 rps / ~25s** against the local Java Spring API. This is not a load against [autotests.ai](https://autotests.ai/) or Box2. Artillery runs `src/*.ts` directly — not `tsc` → JS. Load is **open** `arrivalRate=10` for 60s = **10 HTTP/s** (one HTTP per scenario, not a 5-step flow; not closed 10 VU).
...
# load = artillery arrivalRate=10 HTTP/s for 60s (open, 1 HTTP/scenario), not 10 chains/s, not closed 10 VU, not k6, not Gatling TS, not tsc.
```
