...
test.describe('Backend wiring on deployed stand', { tag: ['@api'] }, () => {
  test('GET /api/health — deployed service is the active backend module, not a neighbour', async ({ request }) => {
  ...
  test('GET /api/items — catalogue is served from PostgreSQL, not a stub or fallback', async ({ request }) => {
    const response = await apiRequest(request, 'GET', '/api/items');
    expect(response.status()).toBe(200);
