...
**Not a pyramid layer:** load — JMeter JMX etalon (`tests-java-jmeter`, living) · Gatling Java DSL sibling (`tests-java-gatling`, living) · slots: JMeter Groovy (`tests-groovy-jmeter`) · Gatling Kotlin/Scala/JS/TS · k6 · Locust · Yandex.Tank. `layers: [performance]` on `/stack/` Tests board. Not `@Layer`.  
