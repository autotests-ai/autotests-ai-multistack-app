...
**Not a pyramid layer:** load — JMeter JMX etalon (`tests-java-jmeter`, living) · JMeter Kotlin TreeBuilder sibling (`tests-kotlin-jmeter`, living) · JMeter JSR223 Groovy sibling (`tests-groovy-jmeter`, living) · Gatling Java DSL sibling (`tests-java-gatling`, living) · Gatling Kotlin DSL sibling (`tests-kotlin-gatling`, living) · Gatling Scala DSL sibling (`tests-scala-gatling`, living) · Gatling JS SDK sibling (`tests-javascript-gatling`, living, not CI) · Gatling TS SDK sibling (`tests-typescript-gatling`, living, not CI) · k6 JavaScript sibling (`tests-javascript-k6`, living, not CI) · k6 TypeScript sibling (`tests-typescript-k6`, living, not CI) · Locust Python sibling (`tests-python-locust`, living, not CI) · Yandex.Tank sibling (`tests-python-yandex_tank`, living, not CI) · Vegeta sibling (`tests-go-vegeta`, living, not CI) · Artillery JS sibling (`tests-javascript-artillery`, living, not CI) · Artillery TS sibling (`tests-typescript-artillery`, living, not CI) · Goose sibling (`tests-rust-goose`, living, not CI) · NBomber sibling (`tests-csharp-nbomber`, living, not CI) · wrk Lua sibling (`tests-lua-wrk`, living, not CI, not wrk2). `layers: [performance]` on `/stack/` Tests board. Not `@Layer`. Teaching job is `load-tests` (knobs `LOAD_LANG`+`LOAD_TOOL`); not `jmeter-*` / `gatling-*` / `k6-*` / `locust-*` / `vegeta-*` / `artillery-*` job names.  
