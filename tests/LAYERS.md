...
**Not a pyramid layer:** load — JMeter JMX etalon (`tests-java-jmeter`, living) · JMeter Kotlin TreeBuilder sibling (`tests-kotlin-jmeter`, living) · JMeter JSR223 Groovy sibling (`tests-groovy-jmeter`, living) · Gatling Java DSL sibling (`tests-java-gatling`, living) · Gatling Kotlin DSL sibling (`tests-kotlin-gatling`, living) · Gatling Scala DSL sibling (`tests-scala-gatling`, living) · k6 JavaScript sibling (`tests-javascript-k6`, living, not CI) · k6 TypeScript sibling (`tests-typescript-k6`, living, not CI) · Locust Python sibling (`tests-python-locust`, living, not CI) · Yandex.Tank sibling (`tests-python-yandex_tank`, living, not CI) · slots: Gatling JS/TS. `layers: [performance]` on `/stack/` Tests board. Not `@Layer`. Teaching job is `load-tests` (knobs `LOAD_LANG`+`LOAD_TOOL`); not `jmeter-*` / `gatling-*` / `k6-*` / `locust-*` job names.  
