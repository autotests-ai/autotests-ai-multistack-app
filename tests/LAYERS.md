...
```
                    ...
                    ├─────────────┤
                    │     e2e     │  user flows, web :8701 + api :8800 / prod
                    ├─────────────┤
...
| `integration` | tests | e.g. `LoginFormTests`, `LoginEmbedTests` | `@Tag("layout")` / `@Tag("mount")` | `./gradlew testIntegration` | web `:8701` |
| `component_browser` | tests | `…/tests/component/` | `@Tag("component")` | `./gradlew testComponent` | catalog `:3000` |
| `e2e` | tests | `…/tests/`, `…/tests/e2e/` | `@Tag("smoke")` | `./gradlew testE2e` | web `:8701` + API `:8800` (CI) / prod (post-deploy) |
