...
def available(config: TestConfig) -> bool:
    ...
    try:
        response = requests.get(_admin(config, "/__admin/scenarios"), timeout=_TIMEOUT_S)
...
def reset_all(config: TestConfig) -> None:
    response = requests.post(_admin(config, "/__admin/scenarios/reset"), timeout=_TIMEOUT_S)
    assert response.status_code == 200, response.text
