...
class TestAuthApi:
    ...
    @pytest.mark.smoke
    def test_login_with_valid_credentials(self, api):
        response = request(
            api, "POST", "/api/auth/login", json={"username": "user1", "password": "password1"}
        )
    ...
    def test_login_with_unknown_username(self, api):
        response = request(
            api,
            "POST",
            "/api/auth/login",
    ...
    def test_login_rejects_empty_credentials(self, api):
        response = request(
    ...
    def test_login_rejects_short_username(self, api):
        response = request(
            api, "POST", "/api/auth/login", json={"username": "ab", "password": "password1"}
        )
        assert response.status_code == 400
        body = response.json()
        assert_schema(body, "error.json")
    ...
    @pytest.mark.negative
    def test_login_rejects_short_password(self, api):
        response = request(
            api, "POST", "/api/auth/login", json={"username": "user1", "password": "123"}
        )
        assert response.status_code == 400
        body = response.json()
        assert_schema(body, "error.json")
    ...
    @pytest.mark.negative
    def test_login_rejects_empty_username(self, api):
        response = request(
            api, "POST", "/api/auth/login", json={"username": "", "password": "password1"}
        )
        assert response.status_code == 400
        body = response.json()
        assert_schema(body, "error.json")
    ...
    @pytest.mark.negative
    def test_login_rejects_empty_password(self, api):
        response = request(
            api, "POST", "/api/auth/login", json={"username": "user1", "password": ""}
        )
        assert response.status_code == 400
        body = response.json()
        assert_schema(body, "error.json")
    ...
    @pytest.mark.negative
    def test_login_rejects_malformed_json(self, api):
        response = request(api, "POST", "/api/auth/login", data="not json")
        assert response.status_code == 400
    ...
    def test_register_new_user(self, api):
        name = username()
        response = request(
            api, "POST", "/api/auth/register", json={"username": name, "password": "password123"}
        )
        ...
        assert body["redirectUrl"] == "/"
        delete_account(api, body["token"])

    @allure.title("POST /api/auth/register accepts a 3-character username and 6-character password")
    def test_register_accepts_minimum_length_credentials(self, api):
        import uuid
        name = uuid.uuid4().hex[:3]
        response = request(
            api, "POST", "/api/auth/register", json={"username": name, "password": "123456"}
        )
        assert response.status_code == 201
        body = response.json()
        assert_schema(body, "auth-response.json")
    ...
    @allure.title("POST /api/auth/login with min-length unknown user is 401, not 400")
    def test_login_min_length_unknown_user_is_unauthorized(self, api):
        import uuid
        response = request(
            api,
            "POST",
    ...
    def test_register_rejects_short_username(self, api):
        response = request(
            api,
            "POST",
            "/api/auth/register",
    ...
    def test_register_rejects_empty_username(self, api):
        response = request(
            api,
            "POST",
            "/api/auth/register",
    ...
    def test_register_rejects_empty_password(self, api):
        response = request(
            api,
            "POST",
            "/api/auth/register",
    ...
    def test_register_rejects_empty_credentials(self, api):
        response = request(
            api, "POST", "/api/auth/register", json={"username": "", "password": ""}
        )
        assert response.status_code == 400
        body = response.json()
        assert_schema(body, "error.json")
    ...
    def test_register_rejects_malformed_json(self, api):
        response = request(api, "POST", "/api/auth/register", data="not json")
        assert response.status_code == 400
        assert response.json()["message"] == "Request body is not valid JSON"

    @allure.title("GET /api/auth/me returns the profile contract for a bearer token")
    def test_profile_with_bearer_token(self, api):
        token = login(api, "user1", "password1")
        response = request(api, "GET", "/api/auth/me", token=token)
        assert response.status_code == 200
        body = response.json()
    ...
    def test_profile_without_token(self, api):
        response = request(api, "GET", "/api/auth/me")
        assert response.status_code == 401

    @allure.title("GET /api/auth/me with a garbage token returns 401")
    def test_profile_with_garbage_token(self, api):
        response = request(api, "GET", "/api/auth/me", token="not-a-jwt")
        assert response.status_code == 401

    @allure.title("POST /api/auth/logout returns 204")
    def test_logout_returns_no_content(self, api):
        response = request(api, "POST", "/api/auth/logout")
        assert response.status_code == 204

    @allure.title("DELETE /api/auth/me without a token returns 401")
    @pytest.mark.negative
    def test_delete_without_token(self, api):
        response = request(api, "DELETE", "/api/auth/me")
        assert response.status_code == 401

    @allure.title("DELETE /api/auth/me with a garbage token returns 401")
    @pytest.mark.negative
    def test_delete_with_garbage_token(self, api):
        response = request(api, "DELETE", "/api/auth/me", token="not-a-jwt")
        assert response.status_code == 401

    @allure.title("DELETE /api/auth/me removes the account: repeated login is rejected")
    def test_delete_removes_account(self, api):
        name = username()
        token = register(api, name, "password123")
        delete_account(api, token)
        response = request(
            api, "POST", "/api/auth/login", json={"username": name, "password": "password123"}
        )
        assert response.status_code == 401
        assert response.json()["message"] == WRONG_CREDENTIALS_MESSAGE

    @allure.title("unmapped /api/* path requires authentication (security catch-all)")
    def test_unmapped_api_path_requires_authentication(self, api):
        response = request(api, "GET", "/api/nope")
        assert response.status_code == 401
