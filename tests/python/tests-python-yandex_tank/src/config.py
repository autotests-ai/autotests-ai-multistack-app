"""Stand + injection knobs for Yandex.Tank (Python + phantom).
...
def profile() -> str:
    return _first_non_blank(os.environ.get("TANK_PROFILE"), "smoke").lower()
...
def allow_public() -> bool:
    return _first_non_blank(os.environ.get("TANK_ALLOW_PUBLIC"), "false").lower() == "true"
...
def refuse_shared_prod(base_url: str) -> None:
    ...
    if shared and not allow_public():
        raise SystemExit(
            ...
            "Pass TANK_ALLOW_PUBLIC=true when the host is a dedicated load stand."
        )
