...
class TestAuthRoundTripApi:
    ...
    def test_account_lifecycle_round_trip(self, api):
        name = username()
        ...
        logged_in = request(
            api, "POST", "/api/auth/login", json={"username": name, "password": password}
        )
        assert logged_in.status_code == 200
        token = logged_in.json()["token"]

        me = request(api, "GET", "/api/auth/me", token=token)
        assert me.status_code == 200
        assert me.json()["username"] == name

        logout = request(api, "POST", "/api/auth/logout", token=token)
        assert logout.status_code == 204

        # Stateless JWT: logout does not invalidate the token server-side — by design.
        still_me = request(api, "GET", "/api/auth/me", token=token)
        assert still_me.status_code == 200
        assert still_me.json()["username"] == name

        deleted = request(api, "DELETE", "/api/auth/me", token=token)
        assert deleted.status_code == 204

        gone = request(api, "GET", "/api/auth/me", token=token)
