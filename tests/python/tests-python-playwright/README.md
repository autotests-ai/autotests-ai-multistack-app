...
Python UI school (Playwright `get_by_test_id`, same `data-testid` as Java/TS Playwright). HTTP in this folder is Playwright **APIRequest**, not httpx. Default CI cell stays [`tests-java-gradle-junit5-allure3-selenide`](../../java/tests-java-gradle-junit5-allure3-selenide/). Sibling Selene / Selenium UI blocks stay in their folders. HTTP-only httpx school stays in [`tests-python-httpx`](../tests-python-httpx/).

## Quick start

```bash
cd tests/python/tests-python-playwright
python -m venv .venv
...
(`projectKey` `autotests-ai-multistack-app-tests-python-playwright`, gate `qa-guru-canon`).
