...
class TestAuthApi:
    ...
    def test_register_accepts_minimum_length_credentials(self, config):
        import uuid
        name = uuid.uuid4().hex[:3]
        response = request(
            config, "POST", "/api/auth/register", json={"username": name, "password": "123456"}
        )
        assert response.status_code == 201
        body = response.json()
        assert_schema(body, "auth-response.json")
    ...
    def test_login_min_length_unknown_user_is_unauthorized(self, config):
        import uuid
        response = request(
            config,
            "POST",
