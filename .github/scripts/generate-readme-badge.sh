...
# Sync copy → consumer .github/scripts/generate-readme-badge.sh (selenoid-tests, multistack_github-pyramid, …)
#
# Usage:
#   generate-readme-badge.sh RESULTS_DIR OUTPUT_DIR BRANCH BUILD \
#     [COMPONENT_FILTER] [TITLE] [STACK_FOOTER] [OUTPUT_SUFFIX] [LOGO_SRC]
#
# OUTPUT_SUFFIX empty → badge.svg; suffix "selenoid" → badge-selenoid.svg
# LOGO_SRC (optional) → copy to OUTPUT_DIR/logo.svg; absent → fallback wordmark.
#   Only the base run (no OUTPUT_SUFFIX) publishes logo.svg.
set -euo pipefail
...
# Publish logo.svg once (base run only): copy committed asset or emit fallback wordmark.
if [ -z "${OUTPUT_SUFFIX}" ]; then
  if [ -n "${LOGO_SRC}" ] && [ -f "${LOGO_SRC}" ]; then
    cp "${LOGO_SRC}" "${OUTPUT_DIR}/logo.svg"
  elif [ ! -f "${OUTPUT_DIR}/logo.svg" ]; then
    logo_initials="$(printf '%.20s' "${TITLE}")"
    cat > "${OUTPUT_DIR}/logo.svg" <<EOF
<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120" role="img" aria-label="${TITLE} logo">
  <title>${TITLE}</title>
  <defs>
    <linearGradient id="logo-bg" x1="0" y1="0" x2="120" y2="120" gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="#0b4f6c"/>
      <stop offset="1" stop-color="#008a56"/>
    </linearGradient>
  </defs>
  <rect width="120" height="120" rx="24" fill="url(#logo-bg)"/>
  <text x="60" y="70" text-anchor="middle" fill="#ffffff" font-family="ui-sans-serif,system-ui,sans-serif" font-size="15" font-weight="700">${logo_initials}</text>
</svg>
EOF
  fi
fi
