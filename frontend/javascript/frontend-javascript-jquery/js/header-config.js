(function (w) {
  ...
  function buildHeaderConfig(lang) {
    var nav = w.I18n.dictionaries[w.I18n.langFromDetail(lang)].nav;
    var envItems = typeof w.envNavItems === 'function' ? w.envNavItems() : [];
    return {
      brand: { href: w.appPath('/'), label: 'Multistack' },
      nav: [
        { href: w.appPath('/'), label: nav.home, testid: 'header-nav-home' },
        { href: w.appPath('/login'), label: nav.login, testid: 'header-nav-login' },
        { href: w.appPath('/register'), label: nav.register, testid: 'header-nav-register' },
  ...
  function startI18n(applyCopy) {
    ...
    function apply(lang) {
      var code = w.I18n.langFromDetail(lang);
      var copy = w.I18n.dictionaries[code];
      w.I18n.applyDocumentLang(code);
      navKey = syncHeaderNav(buildHeaderConfig(code), navKey);
      w.I18n.applyDataI18n(copy);
      if (typeof applyCopy === 'function') {
        applyCopy(copy, code);
      }
    }
    if (w.__i18nOnLangChange) {
      document.removeEventListener(w.I18n.HEADER_LANG_CHANGE, w.__i18nOnLangChange);
    }
    w.__i18nOnLangChange = function (event) {
      apply(event.detail && event.detail.lang);
    };
    document.addEventListener(w.I18n.HEADER_LANG_CHANGE, w.__i18nOnLangChange);
    apply(w.I18n.readStoredLang());
  }
