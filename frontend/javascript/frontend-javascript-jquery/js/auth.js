(function () {
  // Auth surface for the jQuery pages. Transport is fetch, not $.ajax: the API is
  // plain JSON and the same code then reads identically in every non-jQuery module.
  ...
  async function deleteAccount() {
    const token = readLocalStorage(AUTH_TOKEN_KEY);
    if (token) {
      await fetch(window.apiUrl("/auth/me"), {
        method: "DELETE",
        headers: { Authorization: "Bearer " + token },
      }).catch(() => {});
    }
