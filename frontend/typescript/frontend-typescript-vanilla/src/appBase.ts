...
function resolveFromPathname(pathname: string) {
  ...
  // Container publish-port, vite dev and jsdom all serve the pages at the
  // document root, so there is no mount prefix to add.
  return {
    backendId: null as string | null,
...
/** Product mount path — trailing slash omitted. */
