...
function resolveFromPathname(pathname: string) {
  ...
  // Container publish-port, vite dev and jsdom all serve the documents at the root:
  // a mount-shaped prefix would point every link one level too deep.
  return {
    backendId: null as string | null,
...
/** Document prefix for same-origin links — trailing slash omitted. */
