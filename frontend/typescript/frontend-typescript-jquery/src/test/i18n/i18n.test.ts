...
describe('i18n helpers', () => {
  ...
  it('applies html[lang] and keeps the event name aligned with header.js', () => {
    ...
    expect(LANG_STORAGE_KEY).toBe('zds-lang');
  });

  it('fills data-i18n nodes from the copied dictionary', () => {
    document.body.innerHTML = '<span data-i18n="login.title">Login Form</span>';
    applyDataI18n(ru);
    expect(document.body.textContent).toBe(ru.login.title);
  });
});
