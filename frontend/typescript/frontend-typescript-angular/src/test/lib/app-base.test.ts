...
async function loadFor(pathname: string) {
  ...
  vi.resetModules();
  return import('../../app/lib/app-base');
}
...
describe('appBase — mount precedence', () => {
  it('takes backend and frontend from the path matrix', async () => {
    const { APP_BASE, BACKEND_ID, appPath, apiUrl, authTokenStorageKey } = await loadFor(
      '/stack/backend-java-spring/frontend-typescript-angular/login',
    );

    expect(APP_BASE).toBe('/stack/backend-java-spring/frontend-typescript-angular');
    ...
    expect(appPath('/js/header.js')).toBe(
      '/stack/backend-java-spring/frontend-typescript-angular/js/header.js',
    );
  ...
  it('keeps a bare product mount when there is no backend prefix', async () => {
    const { APP_BASE, BACKEND_ID, appPath, apiUrl, authTokenStorageKey } = await loadFor(
      '/stack/frontend-typescript-angular/login',
    );

    expect(APP_BASE).toBe('/stack/frontend-typescript-angular');
    expect(BACKEND_ID).toBeNull();
    expect(authTokenStorageKey()).toBe('authToken');
    expect(appPath('/js/header.js')).toBe('/stack/frontend-typescript-angular/js/header.js');
  ...
  it('scopes auth token keys per backend and shares them across frontends', async () => {
    const spring = await loadFor('/stack/backend-java-spring/frontend-typescript-angular/');
