...
describe('appBase — mount precedence', () => {
  it('takes backend and frontend from the path matrix', async () => {
    const { APP_BASE, BACKEND_ID, appPath, apiUrl, authTokenStorageKey } = await loadFor(
      '/backend-java-spring/frontend-typescript-angular/login',
    );

    expect(APP_BASE).toBe('/backend-java-spring/frontend-typescript-angular');
    ...
    expect(appPath('/js/header.js')).toBe(
      '/backend-java-spring/frontend-typescript-angular/js/header.js',
    );
  ...
  it('keeps a bare product mount when there is no backend prefix', async () => {
    const { APP_BASE, BACKEND_ID, appPath, apiUrl, authTokenStorageKey } = await loadFor(
      '/frontend-typescript-angular/login',
    );

    expect(APP_BASE).toBe('/frontend-typescript-angular');
    expect(BACKEND_ID).toBeNull();
    expect(authTokenStorageKey()).toBe('authToken');
    expect(appPath('/js/header.js')).toBe('/frontend-typescript-angular/js/header.js');
  ...
  it('scopes auth token keys per backend and shares them across frontends', async () => {
    const spring = await loadFor('/backend-java-spring/frontend-typescript-angular/');
