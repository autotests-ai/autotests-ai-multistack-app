...
| `sync-stack-matrix.py` | deploy ports + hub `tests.modules` → `stack/matrix.json` + `js/env-hosts.js` (SSOT: ethalon) |
| `catalog-cd-matrix.py` | Catalog CD services: deploy frontends minus `teaching: true` (SSOT: ethalon) |
