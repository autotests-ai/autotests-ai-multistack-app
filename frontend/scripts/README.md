...
| `vite-overlay-runtime.mjs` | Vite plugin: serve `vendor/ds` (`css`/`js`/`templates`) in `dev`/`preview`. SSOT here; each cell also has `scripts/vite-overlay-runtime.mjs` because Docker build context is the cell. |
