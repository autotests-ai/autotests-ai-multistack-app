...
Native (Android / iOS) cells of the same product are **not** frontend languages — they live
in [`../mobile/`](../mobile/), because an app bundle has no dev server, no `dist/` and no
vhost, and the CI verbs here dispatch on `FRONTEND_LANG` (`typescript` / `javascript`).
