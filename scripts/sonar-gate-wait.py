...
def fetch_ce_task(base_url: str, ce_task_id: str, token: str) -> dict:
    return api_get_json(base_url, "/api/ce/task", {"id": ce_task_id}, token).get("task", {})
    ...
    *,
    ...
    while time.monotonic() < deadline:
        try:
            ...
            if last.get("status", "UNKNOWN") in CE_TERMINAL:
                break
        time.sleep(poll)
...
def run(
    ...
    ce_result = wait_for_ce_task(
        base_url, ce_task_id, token, timeout=timeout, poll=poll
    )
    if not ce_result.get("ok"):
        return ce_result

    gate_result = wait_for_gate(
        base_url, ce_result["analysis_id"], token, timeout=timeout, poll=poll
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--report-task", type=Path)
