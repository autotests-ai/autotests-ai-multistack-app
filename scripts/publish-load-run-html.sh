#!/usr/bin/env bash
# Copy JMeter/Gatling/k6/Locust/Tank HTML to load.autotests.ai/runs/{injector}::{backend}/{run_id}/.
...
resolve_html_dir() {
  ...
  if [ -z "$dir" ]; then
    ...
    case "${LOAD_TOOL:-}" in
      ...
      *)
        echo "STOP: set HTML_DIR or LOAD_TOOL=jmeter|gatling|k6|locust|tank" >&2
        return 1
        ;;
    esac
  fi
...
TOOL_HTML_RE='Apache JMeter Dashboard|[Gg]atling|k6 report|k6-web-dashboard|xk6-dashboard|Grafana k6|[Ll]ocust|Yandex\.Tank'
...
if ! grep -Eq "$TOOL_HTML_RE" "${HTML}/index.html"; then
  echo "STOP: ${HTML}/index.html is not a JMeter/Gatling/k6/Locust/Tank dashboard" >&2
  exit 1
fi
...
if ! grep -Eq "$TOOL_HTML_RE" /tmp/load-run-index.html; then
  echo "STOP: live HTML is not a JMeter/Gatling/k6/Locust/Tank dashboard" >&2
  exit 1
fi
echo "ok ${URL}"
